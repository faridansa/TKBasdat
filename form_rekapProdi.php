<?php
  ...
  function connect() {
    $conn = "dbname=graceangelica user=postgres password=bocahtengil";
  ...
  function selectOptionQuery($type) {
    ...
    if ($type == "periode") {
      ...
      $sql = "SELECT DISTINCT jenjang, nama, jenis_kelas
              FROM SIRIMA.PENERIMAAN_PRODI LEFT OUTER JOIN SIRIMA.PROGRAM_STUDI
              ON kode_prodi = kode
              -- WHERE jenis_kelas = '$type'
              ORDER BY jenis_kelas DESC, jenjang, nama ASC";
    }
